...
ui <- shinyUI(fluidPage(theme = shinytheme("spacelab"),
                        ...
                        sidebarLayout(
                          ...
                          mainPanel(
                            tabsetPanel(
                              tabPanel("Pitch Outcome Distributions", plotOutput("pitch_plot"))
                            )
...
server <- shinyServer(function(input, output, session) {
  ...
  the.big.guy.pitcher.radii <- function(pitcher = "All", other_pitcher = "None") {
    ...
    title = "Comparison Pitch Outcome Distribution"

    if (pitcher != "All") {
      if (other_pitcher != "None") {
        if (other_pitcher == "All"){
          relevant.data <- filter(pitches.model.data.2, pitcher_name == pitcher)
          relevant.data.2 <- filter(pitches.model.data.2, pitcher_name != pitcher)
        }
        else{
          relevant.data <- filter(pitches.model.data.2, pitcher_name == pitcher)
          relevant.data.2 <- filter(pitches.model.data.2, pitcher_name == other_pitcher)
        }
        loginfo(nrow(relevant.data.2))
        loginfo(nrow(relevant.data))
        ...
        if (zones$data == 0 & !(is.null(click_point_x$data))){
          ...
          relevant.data <- relevant.data %>% dplyr::filter(radius < 25)
          relevant.data.2$radius <- create_radii(relevant.data.2$x, relevant.data.2$y, x_val, y_val)
          relevant.data.2 <- relevant.data.2 %>% dplyr::filter(radius < 25)
          loginfo(nrow(relevant.data.2))
          loginfo(nrow(relevant.data))
        }
        ...
        if (zones$data == 0 & !(is.null(click_point_x$data))){
          relevant.data$radius <- create_radii(relevant.data$x, relevant.data$y, x_val, y_val)
          relevant.data <- relevant.data %>% dplyr::filter(radius < 20)
        }
        loginfo(nrow(relevant.data))
      ...
      relevant.data <- filter(pitches.model.data.2)
      loginfo(nrow(relevant.data))
  ...
  output$pitch_plot <- renderPlot({
    list.pitch <- the.big.guy.pitcher.radii(input$pitcher_name, input$other_pitcher)
    list.pitch[[1]]
  })
